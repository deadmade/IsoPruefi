...
public class CombinedLoadTest : LoadTestBase
{
    ...
    public async Task Test_Combined_MQTT_And_REST_API_Load()
    {
        ...
        var restApiScenario = Scenario.Create("rest_api_users_combined", async context =>
        {
            ...
            var request = Http.CreateRequest("GET", $"{GetApiBaseUrl()}/api/v1/TemperatureData/GetTemperature?start={Uri.EscapeDataString(startDate.ToString("O"))}&end={Uri.EscapeDataString(endDate.ToString("O"))}&place={Uri.EscapeDataString(location)}")
        ...
        Console.WriteLine($"  Success Rate: {stats.AllOkCount}/{stats.AllRequestCount} ({stats.AllOkCount * 100.0 / stats.AllRequestCount:F1}%)");
